...
def dcg_at_k(r, k, method=0):
    ...
    if r.size:
        if method == 0:
            ...
            #return np.sum(r / np.log2(np.arange(2, r.size + 2))) -> the same
            return np.sum(gain/discount) #No entiendo porque cuenta en 2
        else:
...
def score_ndcg5(preds, ground_truth, k=5):
    ...
    ground_truth.shape = rows,1 #Unidimensional label
